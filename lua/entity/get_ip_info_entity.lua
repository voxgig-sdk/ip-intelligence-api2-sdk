...
local GetIpInfoEntity = {}
GetIpInfoEntity.__index = GetIpInfoEntity


function GetIpInfoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetIpInfoEntity)
  self._name = "get_ip_info"
  self._client = client
...
function GetIpInfoEntity:get_name()
  return self._name
end


function GetIpInfoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetIpInfoEntity.new(self._client, opts)
end
...
function GetIpInfoEntity:mark_deleted()
  self._deleted = true
end


function GetIpInfoEntity:deleted()
  return true == self._deleted
end


function GetIpInfoEntity:data_set(args)
  if args ~= nil then
...
function GetIpInfoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetIpInfoEntity:match_set(args)
  if args ~= nil then
...
function GetIpInfoEntity:match_get()
...
function GetIpInfoEntity:stream(action, args, callopts)
...
---@param reqmatch GetIpInfoLoadMatch
---@param ctrl? table
---@return GetIpInfo
---@return string? err
function GetIpInfoEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GetIpInfoEntity:_run_op(ctx, post_done)
  local utility = self._utility
