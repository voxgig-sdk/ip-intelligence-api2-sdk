...
class IpIntelligenceApi2MakeError
{
    public static function call(?IpIntelligenceApi2Context $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IpIntelligenceApi2Context([], null);
        }
        $op = $ctx->op ?? new IpIntelligenceApi2Operation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IpIntelligenceApi2Result([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IpIntelligenceApi2Error) ? $err->msg : (string)$err;
        ...
        $sdk_err = new IpIntelligenceApi2Error('', $msg, $ctx);
        ...
        if ($err instanceof IpIntelligenceApi2Error) {
