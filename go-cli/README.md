# ip-intelligence-api2-cli

boru-driven command-line client **and** interactive REPL for the IpIntelligenceApi2
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-intelligence-api2-cli)
make build

# 2. See usage (words, entities, env vars)
./ip-intelligence-api2-cli --help

# 3. Provide credentials once, via the environment
export IP_INTELLIGENCE_API2_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./ip-intelligence-api2-cli load 1 get_ip_info            # {id:1} shorthand
./ip-intelligence-api2-cli load '{id:1}' get_ip_info       # explicit match map

# 5. Override the API base URL for a single call
IP_INTELLIGENCE_API2_BASE=https://api.example.com ./ip-intelligence-api2-cli load 1 get_ip_info

# 6. No arguments -> interactive REPL
./ip-intelligence-api2-cli
ip-intelligence-api2> load 1 get_ip_info
ip-intelligence-api2> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-intelligence-api2-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ip-intelligence-api2-cli load 1 get_ip_info
   ```
...
```sh
./ip-intelligence-api2-cli load 1 get_ip_info          # scalar shorthand for {id:1}
./ip-intelligence-api2-cli load '{id:1}' get_ip_info     # explicit match map
```
...
./ip-intelligence-api2-cli load 1 get_ip_info
```
...
Run with no arguments to open a REPL (prompt `ip-intelligence-api2>`). Each line is
evaluated as its own boru expression:

```text
$ ./ip-intelligence-api2-cli
ip-intelligence-api2> load 1 get_ip_info
ip-intelligence-api2> /help
ip-intelligence-api2> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_ip_info`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ip-intelligence-api2-cli`. |
...
(`./ip-intelligence-api2-cli <expr>`) and interactively (the REPL), and expressions compose the
