// Command IpIntelligenceApi2-cli is a CLI + REPL for the IpIntelligenceApi2 SDK,
...
const prompt = "ip-intelligence-api2"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get_ip_info"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      IP_INTELLIGENCE_API2_APIKEY (api key), IP_INTELLIGENCE_API2_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: IP_INTELLIGENCE_API2_APIKEY carries the API key and
	// IP_INTELLIGENCE_API2_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IP_INTELLIGENCE_API2_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IP_INTELLIGENCE_API2_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewIpIntelligenceApi2SDK(opts)
