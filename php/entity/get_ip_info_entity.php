...
// IpIntelligenceApi2 SDK GetIpInfo entity
...
class GetIpInfoEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new GetIpInfoEntity($this->_client, $opts);
    }

    /**
     * @param GetIpInfo|array $args GetIpInfo data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = IpIntelligenceApi2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return GetIpInfo|array The current GetIpInfo data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of GetIpInfo fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = IpIntelligenceApi2Helpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of GetIpInfo fields).
     */
    ...
    /**
     * Load a single GetIpInfo.
     *
     * @param GetIpInfoLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed GetIpInfoLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return GetIpInfo|array The loaded GetIpInfo as an assoc-array at the
     *   SDK boundary; throws IpIntelligenceApi2Error on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = IpIntelligenceApi2Helpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
