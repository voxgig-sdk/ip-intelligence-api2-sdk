...
func NewContext(ctxmap map[string]any, basectx *Context) *Context {
	...
	if c := getCtxProp(ctxmap, "client"); c != nil {
		if sdk, ok := c.(*IpIntelligenceApi2SDK); ok {
			ctx.Client = sdk
		}
	}
...
func (ctx *Context) MakeError(code string, msg string) *IpIntelligenceApi2Error {
	return NewIpIntelligenceApi2Error(code, msg, ctx)
}
