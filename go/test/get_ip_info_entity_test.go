...
func TestGetIpInfoEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set IPINTELLIGENCEAPI__TEST_GET_IP_INFO_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		if getIpInfoRef01DataDt0Loaded == nil {
...
func get_ip_infoBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("IPINTELLIGENCEAPI__TEST_GET_IP_INFO_ENTID")
	...
	idmapResolved := core.ToMapAny(env["IPINTELLIGENCEAPI__TEST_GET_IP_INFO_ENTID"])
