...
import (
	...
	omni "github.com/voxgig-sdk/ip-intelligence-api2-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IpIntelligenceApi2Error); is {
				out := map[string]any{
					"name":    "IpIntelligenceApi2Error",
...
func omniCtx(arg any, client *sdk.IpIntelligenceApi2SDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IpIntelligenceApi2SDK); is {
			client = live
