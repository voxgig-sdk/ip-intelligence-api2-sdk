...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("IP_INTELLIGENCE_API2_TEST_LIVE") == "TRUE" ||
		os.Getenv("IP_INTELLIGENCE_API2_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("IP_INTELLIGENCE_API2_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.IpIntelligenceApi2SDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.IpIntelligenceApi2Error{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.IpIntelligenceApi2SDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.IpIntelligenceApi2Error{Msg: msg, Code: code}
}
