...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("IPINTELLIGENCEAPI2_TEST_LIVE") == "TRUE" ||
		os.Getenv("IPINTELLIGENCEAPI2_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("IPINTELLIGENCEAPI2_TEST_EXPLAIN"); explain != "" {
