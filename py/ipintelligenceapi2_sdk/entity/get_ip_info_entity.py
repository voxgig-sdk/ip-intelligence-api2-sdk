...
from ipintelligenceapi2_sdk.ipintelligenceapi2_types import (
    GetIpInfo,
    GetIpInfoLoadMatch,
)
...
class GetIpInfoEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return GetIpInfoEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> GetIpInfo:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.GetIpInfo().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
