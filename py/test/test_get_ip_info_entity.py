    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set IPINTELLIGENCEAPI__TEST_GET_IP_INFO_ENTID JSON to run live")
        client = setup["client"]
...
def _get_ip_info_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("IPINTELLIGENCEAPI__TEST_GET_IP_INFO_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("IPINTELLIGENCEAPI__TEST_LIVE") == "TRUE":
    ...
    _live = env.get("IPINTELLIGENCEAPI__TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("IPINTELLIGENCEAPI__TEST_EXPLAIN") == "TRUE",
        "live": _live,
