    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set IP_INTELLIGENCE_API2_TEST_GET_IP_INFO_ENTID JSON to run live")
        client = setup["client"]
        ...
        get_ip_info_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(get_ip_info_ref01_data_dt0_loaded))
        assert get_ip_info_ref01_data_dt0_load_result is not None
        assert get_ip_info_ref01_data_dt0_load_result["id"] == get_ip_info_ref01_data["id"]
...
def _get_ip_info_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("IP_INTELLIGENCE_API2_TEST_GET_IP_INFO_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("IP_INTELLIGENCE_API2_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = IpIntelligenceApi2SDK(helpers.to_map(merged_opts))

    _live = env.get("IP_INTELLIGENCE_API2_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("IP_INTELLIGENCE_API2_TEST_EXPLAIN") == "TRUE",
        "live": _live,
