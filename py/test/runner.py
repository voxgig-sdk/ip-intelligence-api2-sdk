# IpIntelligenceApi2 SDK test runner
...
class IpIntelligenceApi2TestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IpIntelligenceApi2TestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IpIntelligenceApi2TestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IpIntelligenceApi2TestRunner.getenv("IP_INTELLIGENCE_API2_TEST_LIVE")
        override = IpIntelligenceApi2TestRunner.getenv("IP_INTELLIGENCE_API2_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IpIntelligenceApi2TestRunner.getenv(key)
        ...
        explain = IpIntelligenceApi2TestRunner.getenv("IP_INTELLIGENCE_API2_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["IP_INTELLIGENCE_API2_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if IpIntelligenceApi2TestRunner._test_control is not None:
            return IpIntelligenceApi2TestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IpIntelligenceApi2TestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IpIntelligenceApi2TestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IpIntelligenceApi2TestRunner.load_test_control()
...
def load_env_local():
    IpIntelligenceApi2TestRunner.load_env_local()


def env_override(m):
    return IpIntelligenceApi2TestRunner.env_override(m)


def entity_data(v):
    return IpIntelligenceApi2TestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IpIntelligenceApi2TestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IpIntelligenceApi2TestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IpIntelligenceApi2TestRunner.load_test_control()


def live_client_options():
    return IpIntelligenceApi2TestRunner.live_client_options()


def live_delay_ms():
    return IpIntelligenceApi2TestRunner.live_delay_ms()
