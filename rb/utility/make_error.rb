...
module IpIntelligenceApi2Utilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = IpIntelligenceApi2Context.new({}, nil)
    end
    op = ctx.op || IpIntelligenceApi2Operation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || IpIntelligenceApi2Result.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(IpIntelligenceApi2Error) ? err.msg : err.to_s
    ...
    sdk_err = IpIntelligenceApi2Error.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(IpIntelligenceApi2Error)
