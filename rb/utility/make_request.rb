...
module IpIntelligenceApi2Utilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = IpIntelligenceApi2Response.new({})
    ...
    if fetch_err
      ...
      response = IpIntelligenceApi2Response.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = IpIntelligenceApi2Response.new(fetched)
    else
