...
class GetIpInfoEntity
  ...
  def make
    opts = @_entopts.dup
    GetIpInfoEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IpIntelligenceApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetIpInfo, Hash] the current GetIpInfo data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IpIntelligenceApi2Helpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetIpInfo fields)
  def match_get
  ...
  #
  # @param reqmatch [GetIpInfoLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GetIpInfo.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GetIpInfo, Hash] the loaded GetIpInfo; raises IpIntelligenceApi2Error on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IpIntelligenceApi2Helpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
