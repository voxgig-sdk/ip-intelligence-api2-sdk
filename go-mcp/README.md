# ip-intelligence-api2-mcp

[MCP](https://modelcontextprotocol.io) server exposing the IpIntelligenceApi2 SDK as
two agent tools — `ip-intelligence-api2_list` and `ip-intelligence-api2_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ip-intelligence-api2-mcp)
make build

# 2. Provide credentials via the environment
export IP_INTELLIGENCE_API2_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user ip-intelligence-api2 \
  -- /absolute/path/to/ip-intelligence-api2-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./ip-intelligence-api2-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// ip-intelligence-api2_list: first page of records
{ "entity": "get_ip_info" }
{ "entity": "get_ip_info", "query": { } }

// ip-intelligence-api2_load: one record by id
{ "entity": "get_ip_info", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ip-intelligence-api2-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ip-intelligence-api2 \
     -- "$PWD"/dist/*/ip-intelligence-api2-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ip-intelligence-api2_list` and `ip-intelligence-api2_load` tools now appear
   in new sessions. Ask the agent to *"list get_ip_info using ip-intelligence-api2"*
   and it calls `ip-intelligence-api2_list` with `{"entity":"get_ip_info"}`.
...
```sh
export IP_INTELLIGENCE_API2_APIKEY=sk_live_xxx            # API key
export IP_INTELLIGENCE_API2_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ip-intelligence-api2_list` tool
...
### Call the `ip-intelligence-api2_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ip-intelligence-api2_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ip-intelligence-api2_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `IP_INTELLIGENCE_API2_APIKEY` | API key sent with every request. |
| `IP_INTELLIGENCE_API2_BASE` | Optional override of the API base URL. |
...
```sh
./ip-intelligence-api2-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"ip-intelligence-api2_load","arguments":{"entity":"get_ip_info","query":{"id":1}}}}'
```
