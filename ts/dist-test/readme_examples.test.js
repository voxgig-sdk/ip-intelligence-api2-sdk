...
const __1 = require("..");
const SDK_NAME = 'IpIntelligenceApi2SDK';
...
const TEST_SEED = { "entity": { "get_ip_info": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+IpIntelligenceApi2SDK\b/.test(code) ||
        /\bIpIntelligenceApi2SDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+IpIntelligenceApi2SDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/IpIntelligenceApi2SDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
